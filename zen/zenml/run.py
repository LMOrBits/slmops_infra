...
from zenml.config import load_config
...
@click.option(
    ...
    type=str,
    required=True,
    help="HuggingFace dataset name to ingest"
)
@click.option(
    "--bucket",
    type=str,
...
@click.option(
    "--config",
    type=str,
    default="configs/data_ingestion_config.yaml",
...
def main(
    ...
    Args:
        ...
        dataset: HuggingFace dataset name to ingest
    ...
    # Load config using ZenML's config loader
    pipeline_config = load_config(DataIngestionConfig, config)
    
    # Run pipeline with validated config
    data_ingestion_pipeline(config=pipeline_config)
